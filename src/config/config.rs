...
use crate::{
    config::{APP_NAME, DEFAULT_DEBOUNCE_DURATION, DEFAULT_ROOT_DIR_NAME, SETTINGS_FILE_NAME, SUPPORTED_DRIVES_LINK}, path::{AbsPath, Local, Remote}
};
...
impl Config {
    pub async fn load() -> Arc<Config> {
        ...
        let file_content = tokio::fs::read_to_string(&path).await.unwrap_or_else(panic_required_file(path.as_path()));
...
impl From<String> for DriveKind {
    fn from(s: String) -> Self {
        ...
        if let Ok(scheme) = parse_res && !matches!(scheme, opendal::Scheme::Custom(_)) {
            return Self(scheme)
        } 
        panic!("
            Failed to parse drive kind `{}`.
                ...
                Err: {}
        ", s.red(),SUPPORTED_DRIVES_LINK,parse_res.unwrap_err())
...
pub fn panic_required_file<T>(path: impl Into<AbsPath<Local>>) -> impl FnOnce(io::Error) -> T {
    move |err: io::Error| {
        ...
        match err.kind() {
            io::ErrorKind::NotFound => panic!("File not found: {path}\n Are sure {file_name} exists?"),
