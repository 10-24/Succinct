...
impl Config {
    pub async fn create_ignore(&self) -> Gitignore {
        let ignore_path = self.local.root_path.child(IGNORE_FILE_NAME);
        let ignore_file = fs::File::open(ignore_path.as_ref())
            .await
            .unwrap_or_else(panic_required_file(ignore_path));
